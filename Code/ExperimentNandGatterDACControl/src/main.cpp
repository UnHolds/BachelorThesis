...
void loop()
{
    ...
    if(input[0] == 'A'){
        setVoltage(VCC_DAC_CS, input.substring(1).toInt());
        Serial.print("Setting DAC A voltate to: ");
        Serial.println(input.substring(1).toInt());
    }
    if(input[0] == 'B'){
        setVoltage(INPUT_DAC_CS, input.substring(1).toInt());
        Serial.print("Setting DAC B voltate to: ");
        Serial.println(input.substring(1).toInt());
    }
    if(input[0] == 'R'){
        reportMeasurement(0,0);
    }
}
